...
do
    ...
    do
        ...
        (time -f "%E %P %M" make -j$c) &> ../busybox-$c-$(date +%s).log
        cd ..
        ...
        #cd linux-4.12
        #(make defconfig) &> /dev/null
        #(time -f "%E %P %M" make -j$c) &> ../linux-$c-$(date +%s).log
        #cd ..
        #rm -rf linux-4.12
        ...
        cd coreutils-8.27
        (./configure) &> /dev/null
        (time -f "%E %P %M" make -j$c) &> ../coreutils-$c-$(date +%s).log
        cd ..
        ...
        cd Python-3.6.1
        (./configure) &> /dev/null
        (time -f "%E %P %M" make -j$c) &> ../python3-$c-$(date +%s).log
        cd ..
        rm -rf Python-3.6.1
        ...
        cd Python-2.7.13
        (./configure) &> /dev/null
        (time -f "%E %P %M" make -j$c) &> ../python2-$c-$(date +%s).log
        cd ..
        rm -rf Python-2.7.13
    done
done
